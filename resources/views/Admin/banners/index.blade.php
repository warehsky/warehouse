                ...
                <tbody>
                    @foreach($banners as $key => $item)
                        <tr data-entry-id="{{ $item->id }}">
                            ...
                            <td>
                                {{ $item->autoplay ? 'авто':'ручная' }}
                            </td>
                            <td>
                            {{ $item->public ? 'опубликован' : 'не опубликован' }}
                            </td>
                            <td>
                                <a class="btn btn-xs btn-primary" href="{{ route('bannerItems.index', ['bannerId'=>$item->id]) }}">
                                    состав
                                </a>
                            @if(auth()->guard('admin')->user()->can('banners_edit'))
                                <a class="btn btn-xs btn-info" href="{{ route('banners.edit', $item->id) }}">
                                    изменить
                                </a>

                                <form action="{{ route('banners.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Уверен');" style="display: inline-block;">
