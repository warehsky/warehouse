...
<div class="card">
    <div class="card-header">
        Редактировать/добавить ответ на отзыв о сервисе
    </div>
    <div class="card-header">
        [{{$comment->id}}]{{$comment->comment}}
    </div>
    <div class="card-body">
        <form action="{{ is_object($answer)?route("commentFirmAnswer.update", [$answer->id]):route("commentFirmAnswer.store") }}" method="POST" enctype="multipart/form-data">
            @csrf
            @if(is_object($answer))
                @method('PUT')
            @else
