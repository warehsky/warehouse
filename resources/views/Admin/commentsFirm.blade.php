...
<input type="hidden" id="api_token" value="{{$api_token}}">
    ...
    <th>действия</th>
    <tbody>
        @foreach($commentsFirm as $comment)
            <tr>
                <td>{{$comment->id}}</td>
                ...
                <td id="ac{{$comment->id}}" answerid="{{count($comment->answers)>0?$comment->answers[0]->id:0}}">
                    ...
                    @if(count($comment->answers)>0)
                        <a class="btn bg-info" href="/admin/commentFirmAnswer/{{$comment->answers[0]->id}}/edit?commentId={{$comment->id}}">ответ</a>
                    @else
                        <a class="btn bg-info" href="/admin/commentFirmAnswer/create?commentId={{$comment->id}}">ответить</a>
                    @endif
                </td>
            </tr>
        @endforeach
...
function setComment(id, stat){
    ...
    $.ajax({
        ...
        url: '/Api/setCommentFirm',         /* Куда пойдет запрос */
